...
from twcalendar import TaiwanHolidayCalendar
...
def get_today_tw_ymd_str() -> str:
    """
    回傳台灣時間今天的字串（格式為 YYYYMMDD），例如 20250729
    """
...
def is_tw_holiday(date: datetime = None) -> bool:
    """
    判斷是否為台灣國定假日（非交易日），含補假
    使用 twcalendar 套件
    """
    cal = TaiwanHolidayCalendar()
    target_date = (date or get_today_tw()).date()
    return not cal.is_working_day(target_date)
