...
def generate_script():
    ...
    try:
        ...
        with open('data/script.txt', 'w', encoding='utf-8') as f:
            f.write(script)
        logger.info("Grok 3 script generated successfully")
        return script
    except Exception as e:
...
def text_to_audio(script):
    ...
    try:
        ...
        result = subprocess.run(
            ['edge-tts', '--voice', 'zh-TW-YunJheNeural', '--text', script, '--write-media', output_file, '--rate=+30%'],
        ...
        if not os.path.exists(output_file):
            logger.error(f"Output audio file not generated: {output_file}")
            return None
