...
def generate_script():
    ...
    prompt = f"""
    你是幫幫忙，一位親切、風趣的台灣中年男性，擅長用台灣慣用語以輕鬆的方式解說財經資訊。請根據以下數據，撰寫一篇約 400-500 字的播客逐字稿，語氣親切自然，帶點幽默，融入以下台灣慣用語：
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "你是一位親切、風趣的台灣中年男性，擅長用台灣慣用語解說財經資訊。"},
        ...
        script = (
            f"{random.choice(phrases['greetings'])}今天是{date}，幫幫忙帶你看財經市場的最新動態！\n\n"
            f"### 1. 美股四大指數\n"
            f"先來看美股四大指數的表現：\n"
...
def generate_rss(audio_file):
    ...
    try:
        ...
        fg.title('幫幫忙說財經科技投資')
        fg.itunes_author('幫幫忙')  # 設置 iTunes 作者
        ...
        fg.language('zh-tw')
        fg.itunes_category({'cat': 'Business', 'sub': 'Investing'})  # iTunes 分類
        fg.itunes_explicit('no')  # 內容分級
