...
def fetch_financial_data():
    ...
    for stock in trending_stocks:
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                hist = ticker.history(period='2d')
                if len(hist) < 2:
                    logger.error(f"嘗試 {attempt}：{stock} 數據不足")
                    if attempt == attempts:
                        return None
                    time.sleep(2)
                    continue
                ...
                logger.info(f"獲取熱門股票 {stock}：收盤={close}, 漲跌={change}%")
                break
            except Exception as e:
                logger.error(f"嘗試 {attempt}：獲取 {stock} 失敗：{e}")
                if attempt == attempts:
                    logger.error(f"{stock} 在 {attempts} 次嘗試後失敗")
                    return None
                time.sleep(2)
    ...
    logger.info(f"資金流向：{data['fund_flow']}")
    
    return data

# 獲取 AI 新聞
def fetch_ai_news():
    logger.info("開始獲取 AI 新聞")
    try:
        ...
        if articles:
            ...
            logger.info(f"AI 新聞獲取成功：{news}")
            return news
        logger.warning("未找到 AI 新聞")
        return "今天沒有特別的 AI 新聞，但 AI 持續改變世界！"
    except Exception as e:
        logger.error(f"獲取 AI 新聞失敗：{e}")
        return "今天沒有特別的 AI 新聞，但 AI 持續改變世界！"

# 獲取經濟新聞
def fetch_economic_news():
    logger.info("開始獲取經濟新聞")
    try:
        ...
        if articles:
            news = articles[0]['title'] + " - " + articles[0]['description'][:100] + "..."
            logger.info(f"經濟新聞獲取成功：{news}")
            return news
        logger.warning("未找到經濟新聞")
        return "今天沒有特別的經濟新聞，市場穩穩走！"
    except Exception as e:
        logger.error(f"獲取經濟新聞失敗：{e}")
        return "今天沒有特別的經濟新聞，市場穩穩走！"
...
def generate_script():
    ...
    try:
        ...
        script = (
            ...
            f"### 3. 比特幣與黃金\n"
            f"加密貨幣和商品市場：\n"
            f"- 比特幣（BTC-USD）報價 {data['crypto_commodities']['BTC-USD']['close']} 美元，{'漲' if data['crypto_commodities']['BTC-USD']['change'] >= 0 else '跌'} {abs(data['crypto_commodities']['BTC-USD']['change'])}%，\n"
            f"- {'市場熱度高' if data['crypto_commodities']['BTC-USD']['change'] > 1 else '穩中求進'}。\n"
            f"- 黃金期貨（GC=F）報價 {data['crypto_commodities']['GC=F']['close']} 美元，\n"
            f"- {'漲' if data['crypto_commodities']['GC=F']['change'] >= 0 else '跌'} {abs(data['crypto_commodities']['GC=F']['change'])}%，{'避險需求強' if data['crypto_commodities']['GC=F']['change'] > 0 else '市場偏向風險資產'}。\n"
