...
def generate_script(cmc_api_key, newsapi_key):
    try:
        ...
        indices = fetch_indices() or {'^DJI': {'close': 44371.51, 'change': -0.63}, '^IXIC': {'close': 20585.53, 'change': -0.22}, '^GSPC': {'close': 6259.75, 'change': -0.33}, '^SOX': {'close': 5696.29, 'change': -0.21}, 'QQQ': {'close': 554.20, 'change': -0.23}, 'SPY': {'close': 623.62, 'change': -0.35}}
        btc = fetch_crypto(cmc_api_key) or {'price': 118100, 'change': 4.03}
        gold = fetch_gold() or {'price': 3354.76, 'change': 0.92}
        stocks = fetch_top_stocks() or ['WLGS', 'ABVE', 'BTOG', 'NCNA', 'OPEN']
        news = fetch_news(newsapi_key) or {'ai': {'title': 'Capgemini收購WNS', 'summary': '以33億美元強化企業AI能力！'}, 'economic': {'title': '美國經濟增長放緩', 'summary': '聯準會降息預期降溫！'}}
...
《大叔說財經科技投資》 - {date}
開場白
{greeting}今天是{date}，星期六，雖然美股昨天收盤，但財經世界沒停下來！咱們來盤點昨天的市場動態，包含美股、加密貨幣、黃金、熱門股、AI和美國經濟新聞，還有一句金句幫大家充電！準備好了嗎？一起來瞧瞧！
...
def text_to_audio(script, output_file):
    try:
        ...
        # 使用 ffmpeg 加速語速至 1.3 倍（需在 GitHub Actions 安裝 ffmpeg）
