...
def text_to_audio(script):
    ...
    try:
        # Verify edge-tts is installed using --list-voices
        edge_tts_check = subprocess.run(['edge-tts', '--list-voices'], capture_output=True, text=True)
        if edge_tts_check.returncode != 0:
            logger.error(f"edge-tts not installed or inaccessible: {edge_tts_check.stderr}")
            return None
        logger.debug(f"edge-tts voices: {edge_tts_check.stdout.strip()}")
        
        # Clean script to remove problematic characters
        script = ''.join(c for c in script if c.isprintable())
        if len(script) > 10000:
            logger.warning(f"Script too long ({len(script)} characters), truncating")
            script = script[:10000]
        logger.debug(f"Script length after cleaning: {len(script)} characters")
        logger.debug(f"Script content (first 100 chars): {script[:100]}")
        
        # Create temporary file in data/logs
        temp_file_path = f"data/logs/script_{date}.txt"
        ...
        with open(temp_file_path, 'r', encoding='utf-8') as f:
            file_content = f.read()
            logger.debug(f"Temporary file content length: {len(file_content)} characters")
            logger.debug(f"Temporary file content (first 100 chars): {file_content[:100]}")
            if not file_content:
                logger.error("Temporary file is empty")
                return None
        
        voices = ['zh-TW-YunJheNeural', 'zh-TW-HsiaoYuNeural']
        for voice in voices:
            for attempt in range(1, 4):
                try:
                    logger.info(f"Attempt {attempt}: Running edge-tts with voice {voice} and file {temp_file_path}")
