...
class QuantityStrategy(bt.Strategy):
    ...
    def notify_order(self, order):
        if order.status in [order.Completed]:
            ...
            logger.warning(f"訂單失敗: {order.status}, 原因={order.info.get('reason', '未知')}, 剩餘資金={self.broker.getcash():.2f}")
            self.order = None
    ...
    def notify_trade(self, trade):
        if trade.isclosed:
            ...
            if trade.pnl > 0:
                self.win_count += 1
            logger.info(f"交易結束: PnL={trade.pnl:.2f}, 當前勝率={self.win_count/self.total_trades:.2f}")
