...
def ask_grok(prompt: str, role: str = "user", model: str = "grok-1") -> str:
    """
    呼叫 Grok API，取得純文字回應（適用文字腳本）
    """
    if not GROK_API_URL or not GROK_API_KEY:
        raise EnvironmentError("請設定 GROK_API_URL 與 GROK_API_KEY 環境變數")
...
def ask_grok_json(prompt: str, role: str = "user", model: str = "grok-1") -> dict:
    """
    呼叫 Grok API，取得 JSON 回傳（會自動解析回傳內容為 dict）
    """
    ...
    try:
        ...
        raise ValueError("Grok 回傳內容不是合法 JSON 格式：\n" + reply)