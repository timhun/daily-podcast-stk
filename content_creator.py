...
【腳本內容比例指引】
  第1段（開場+市場總基調）：約250字
  第2-5段（AI Agent動態，2-4則新聞，每則260-280字）：約1200-1500字（70%核心區塊）
...
【腳本內容比例指引】
  第1段（開場+市場總基調）：約80字
  第2-5段（AI Agent動態，2-4則新聞，每則120-180字）：約600-800字（70%核心區塊）
  第6-7段（投資啟示+MA20策略訊號）：約150-200字（30%區塊）
  第8段（金句收尾）：約40字
  總字数目標：800-1100字（普通話廣東發音，7分鐘約需此量）
...
【黃金內容比例：70/30】
- 70% 篇幅：AI Agent 最新動態（2-4則新聞，每則展開成完整段落，含事件+意義）
...
以下新聞已幫您過濾為半導體與 AI 相關，請將每則新聞展開成 60-80 字的完整段落，
包含「發生了什麼關鍵事件」+「對投資大眾的意義或影響」的敘述：
{news_str}
...
第2-5段（AI 新聞，70% 區塊）：
  - 每則新聞獨立成段（60-80字），結構：「事件描述」→「對你的投資意味着什麼」
...
def generate_script_with_llm(prompt, system_prompt=None):
    ...
    result=call_nim(prompt=prompt,task_type="script",temperature=0.7,
                    max_tokens=8000,system=system_prompt)  # ~2000中文字 = 7分鐘普通話
    if result:
