...
def generate_script(market_data, mode, strategy_results, market_analysis):
    ...
    analysis = "\n".join([f"{symbol}: 收盤 {info.get('close', 0):.2f}, 漲跌 {info.get('change', 0):.2f}%"
    ...
    # --- 修正點 1: 處理 XAI_API_KEY 未設定的 Fallback ---
    if not XAI_API_KEY:
        logger.warning("XAI_API_KEY not set, using fallback script")
        # 這裡的 return 字串前面和後面都必須使用三個雙引號 """
        return f"""
        生成 {mode.upper()} 投資大師的文字稿，長度控制在{config['podcast']['script_length_limit']}字內，風格專業親和，使用台灣用語。
        ...
        """ 
        # 原本在 if 區塊結尾多了一個單獨的 """，已移除。

    # --- 修正點 2: 處理 Prompt 變數的定義 ---
    # 這裡必須使用三個雙引號 """ 來定義多行 f-string
    ...
    try:
        ...
        # 變數名稱應該是 Prompt (大寫 P)，而不是 prompt (小寫 p)
