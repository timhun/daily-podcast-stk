...
class StrategyEngine:
    ...
    def daily_backtest(self, mode='tw'):
        ...
        for symbol in symbols:
            try:
                _, daily_df, _, _ = fetch_market_data(symbol, period='1y')
                ...
                daily_df = pd.DataFrame({
                    'close': [22987.92 if symbol == '^TWII' else 196.8 if symbol == '0050.TW' else 1110.0 if symbol == '2330.TW' else 100.0] * 252,
                    'volume': [np.random.randint(2000000, 6000000) if symbol == '^TWII' else np.nan] * 252
                }, index=pd.date_range(start='2025-01-01', periods=252))
            
            market_dir = config['data_paths']['market']
            os.makedirs(market_dir, exist_ok=True)
            csv_file = f"{market_dir}/daily_{symbol.replace('^', '').replace('.', '_')}.csv"
            daily_df.to_csv(csv_file)
